...
public class NeuralNetworkViewer : MonoBehaviour
{
	...
	[Header("Requirement Ref")] [SerializeField]
	...
	[Header("Display Values")] [SerializeField]
	float layerSpacing;
	...
	public void Init(Agent _agent)
	{
		...
		axones = new AxonDisplay[tempNet.layers.Length - 1][][];

		for (x = 0; x < tempNet.layers.Length - 1; x++)
		{
			axones[x] = new AxonDisplay[tempNet.layers[x]][];

			for (y = 0; y < tempNet.layers[x]; y++)
			{
				axones[x][y] = new AxonDisplay[tempNet.layers[x + 1]];

				for (z = 0; z < tempNet.layers[x + 1]; z++)
				{
					axones[x][y][z]         = new AxonDisplay();
					axones[x][y][z].axonObj = Instantiate(axonPrefab, viewerGroup);
					axones[x][y][z].Init(neurons[x][y].neuronRectTransform, neurons[x + 1][z].neuronRectTransform, axonThickness, neuronDiameter);
				}
			}
		}
	...
	public void RefreshAxons()
	{
		for (x = 0; x < axones.Length; x++)
		{
			for (y = 0; y < axones[x].Length; y++)
			{
				...
				{
					axones[x][y][z].Refresh(agent.net.axones[x][z][y], axonColorGradient);
				}
			}
		}
	}
